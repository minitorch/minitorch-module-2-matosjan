...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        return t1.f.add_zip(t1.f.mul_zip(sig, grad_output), t1.f.neg_map(t1.f.mul_zip(t1.f.mul_zip(sig, sig), grad_output)))
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        ...
        permuted_tensor = a._tensor.permute(*order)
        return minitorch.Tensor.make(permuted_tensor._storage, permuted_tensor.shape, permuted_tensor.strides, a.backend)


    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        (order,) = ctx.saved_tensors
        grad = grad_output._tensor.permute(*np.argsort(order))
        return minitorch.Tensor.make(grad._storage, grad.shape, grad.strides,
